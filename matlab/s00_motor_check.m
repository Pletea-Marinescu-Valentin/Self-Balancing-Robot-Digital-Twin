...
if isempty(info.frame)
    ...
    elseif strlength(info.text) > 0
        ...
        msg = [msg sprintf([ ...
            '  %d bytes arrived but none formed a valid frame. SERIAL_BAUD in\n' ...
            '  config.h and the baud in SbrLink.m disagree; both want 115200.\n'], ...
            info.nBytes)];
    end
...
elseif ~isempty(died)
    ...
    switch cause
        ...
        case 'wdt'
            fprintf(2, ['  MATLAB stalled for over 250 ms and the firmware cut the\n' ...
                '  motors, as designed. Close heavy background work.\n']);
        case 'tilt'
